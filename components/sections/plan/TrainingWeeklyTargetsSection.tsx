...
const toLabelSuffix = (value: string): string => `${value.charAt(0).toUpperCase()}${value.slice(1)}`;
...
export const TrainingWeeklyTargetsSection: React.FC<TrainingWeeklyTargetsSectionProps> = ({ selectedDate }) => {
  ...
  const { plans, trainingPlanSettings, dailyTrainingTracking } = useStorage();
  ...
  const weeklyTipProgress = useMemo(() => {
    ...
    /*
     * Raw training lagras per dag.
     * Veckans progress räknas från
     * de dagliga registreringarna.
     */
    const weekEntries = Object.entries(dailyTrainingTracking)
    ...
    const tipsWithTargets = plans.training.filter(goal => Boolean(goal.tipId) && hasAnyTarget(goal.tipId));
  ...
  }, [dailyTrainingTracking, plans.training, selectedDate, t, trainingPlanSettings]);

  const fulfilledTips = useMemo(() => weeklyTipProgress.tips.filter(tip => tip.isFulfilled), [weeklyTipProgress.tips]);

  const inProgressTips = useMemo(() => weeklyTipProgress.tips.filter(tip => !tip.isFulfilled), [weeklyTipProgress.tips]);

  const buildTrainingBadges = (tipId: string, userSettings: (typeof trainingPlanSettings)[string]): TrainingBadgeItem[] => {
