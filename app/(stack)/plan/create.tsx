...
export default function CreatePlanScreen() {
  ...
  const buildSupplementKey = (planName: string, preferredTime: string, supName: string) => `supp:${planName}:${preferredTime}:${supName}`;
  ...
  const newSupplementItems = useMemo(
    () =>
      tempPlans
  ...
  const newTrainingItems = useMemo(
    () =>
      tempPlans
  ...
  const newNutritionItems = useMemo(
    () =>
      tempPlans
  ...
  const duplicateSummary = useMemo(() => {
    ...
    const existingSupplementKeys = new Set(
      (plans.supplements ?? []).flatMap(plan => (plan.supplements ?? []).map(sup => buildSupplementKey(plan.name, plan.prefferedTime, sup.supplement.name)))
    );
  ...
  const existingSupplementItems = useMemo(
    () => (plans ? (plans.supplements ?? []).flatMap(plan => (plan.supplements ?? []).map(sup => `${sup.supplement.name} • ${plan.prefferedTime})`)) : []),
    [plans]
  );

  const existingTrainingItems = useMemo(() => (plans ? (plans.training ?? []).map(tip => tipTitleById(tip.tipId)) : []), [plans, tipTitleById]);

  const existingNutritionItems = useMemo(() => (plans ? (plans.nutrition ?? []).map(tip => tipTitleById(tip.tipId)) : []), [plans, tipTitleById]);

  const existingOtherItems = useMemo(() => (plans ? (plans.other ?? []).map(tip => tipTitleById(tip.tipId)) : []), [plans, tipTitleById]);
  ...
  const renderNewList = (items: { key: string; label: string }[], category: keyof ApprovalsState) =>
    items.length ? (
      items.map((item, idx) => {
        const checked = approvals[category]?.[item.key] ?? true;
        return <Checkbox key={`${item.key}-${idx}`} checked={checked} onPress={() => toggleApproval(category, item.key)} label={item.label} />;
      })
    ) : (
  ...
  const renderExistingList = (items: string[]) => {
    return items.length ? (
      items.map((item, idx) => (
        <ThemedText key={`${item}-${idx}`} type="explainer" style={styles.listItem}>
          •{item}
        </ThemedText>
      ))
    ) : (
      <ThemedText type="explainer" style={styles.listItem}>
        {t('createPlan.none')}
      </ThemedText>
    );
  };
    ...
    if (tempPlans) {
      ...
      const mergeSupplementPlans = (existing: typeof filteredPlans.supplements, incoming: typeof filteredPlans.supplements) => {
        ...
        incoming.forEach(plan => {
          ...
          const existingKeys = new Set((current.supplements ?? []).map(sup => buildSupplementKey(plan.name, plan.prefferedTime, sup.supplement.name)));
  ...
  const buildSectionTitle = React.useCallback(
    (selected: number, total: number, selectedKey: string, defaultKey: string) =>
      selected < total ? t(selectedKey, { selected, total }) : t(defaultKey, { count: total }),
    [t]
  );

  const supplementsTitle = buildSectionTitle(newSupplementSelectedCount, newSupplementCount, 'createPlan.aiSupplementsSelected', 'createPlan.aiSupplements');
  const trainingTitle = buildSectionTitle(newTrainingSelectedCount, newTrainingCount, 'createPlan.aiTrainingSelected', 'createPlan.aiTraining');
  const nutritionTitle = buildSectionTitle(newNutritionSelectedCount, newNutritionCount, 'createPlan.aiNutritionSelected', 'createPlan.aiNutrition');
  const otherTitle = buildSectionTitle(newOtherSelectedCount, newOtherCount, 'createPlan.aiOtherSelected', 'createPlan.aiOther');
    ...
    <Container background="gradient" showBackButton onBackPress={() => router.replace('/(tabs)/plan')}>
      <Card style={styles.card}>
        <View style={styles.title}>
          <GradientText>{t('createPlan.title')}</GradientText>
        </View>

        {/* Målval */}

        <ThemedText type="label" style={globalStyles.marginBottom8}>
          {t('common:createPlan.goals.title')}
        </ThemedText>
        ...
        <GoldenGlowButton
          ...
          accessibilityHint={tempPlans ? t('createPlan.createAIPlanHint') : undefined}
        ...
        {tempPlans?.reasonSummary?.text && (
          <>
            ...
            <ThemedText type="default" style={styles.reasonText} numberOfLines={showAllReason ? undefined : 5}>
              {tempPlans.reasonSummary.text}
            </ThemedText>
            {reasonTooLong && (
              <ShowAllButton
          ...
          <>
            <Card>
              ...
              >
                ...
                <ThemedText type="caption" style={[styles.selectionSummary, styles.srOnly]} accessibilityLiveRegion="polite">
                  {supplementsSummary}
                </ThemedText>
              </Collapsible>
            </Card>
            ...
            <View style={styles.buttonRow}>
              <AppButton title={t('createPlan.acceptAIPlan')} onPress={handleAccept} glow />
              <AppButton title={t('general.cancel')} onPress={handleCancel} variant="secondary" />
            </View>
          </>
        )}
      </Card>
...
const SectionHeader = ({
  ...
  return (
    <View style={styles.sectionHeaderRow}>
      <View style={[styles.sectionHeaderLine, { backgroundColor: isDimmed ? colors.borderLight : colors.primary }]} />
      <ThemedText type="defaultSemiBold" style={[styles.sectionHeaderText, isPrimary && { color: colors.primary }, isDimmed && { color: colors.textMuted }]}>
        {children}
      </ThemedText>
