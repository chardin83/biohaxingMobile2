...
const DayEdit: React.FC<DayeEditProps> = ({
  ...
  const [editingSupplement, setEditingSupplement] =
    useState<SupplementTime | null>(null);
  const [isPlanPickerVisible, setIsPlanPickerVisible] = useState(false);
  const [activeTab, setActiveTab] = useState<"supplements" | "meal">(
    "supplements"
  );
  ...
  const hasMealsToday =
    useStorage().dailyNutritionSummaries[selectedDate]?.meals?.length > 0;
  ...
  const editSupplement = (time: string, supplementName: string) => {
    ...
    const isEditingSupplement = selectedSupplements.find(
      (item) => item.name === supplementName && item.time === time
    );
  ...
  const saveSelectedSupplement = async (supplement: Supplement) => {
    ...
    const supplementExists = selectedSupplements.some(
      (existingSupplement) =>
        existingSupplement.name === supplement.name &&
        existingSupplement.time === time
    );
    ...
    if (!isEditing) {
      updatedSupplements = [
        ...selectedSupplements,
        { ...supplement, time } as SupplementTime,
      ];
    } else {
      updatedSupplements = selectedSupplements.map((existingSupplement) =>
        existingSupplement.name === supplement.name &&
        existingSupplement.time === editingSupplement?.time
      ...
      <View style={styles.container}>
        <View style={styles.tabContainer}>
          <TouchableOpacity
            style={[
              styles.tabWrapper,
              activeTab === "supplements" && styles.activeTabWrapper,
            ]}
          ...
          <TouchableOpacity
            style={[
              styles.tabWrapper,
              activeTab === "meal" && styles.activeTabWrapper,
            ]}
            ...
            <View style={styles.tabContent}>
              ...
              {hasMealsToday && (
                <View
                  style={[
                    styles.badge,
                    { backgroundColor: Colors.dark.checkmarkMeal },
                  ]}
                />
              )}
            </View>
        ...
        {activeTab === "meal" && (
          <NutritionLogger selectedDate={selectedDate} />
        )}
      </View>
    </KeyboardAvoidingView>
  );
